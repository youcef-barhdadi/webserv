...
jump:
			...
			serverbuff.set_server_name(name);
			while(std::getline(ifs, buffer))
			{
				if (buffer[0] == '#' || !buffer.size())
					continue ;
				...
				{
					try{
						std::vector<std::string> portvec = split(vec[1], ',');
				...
				else if (vec.size() == 3 && vec[0] == "error_page")
				{
					...
					err.root = vec[2];
					std::vector<std::string> vec2 = split(vec[1], ',');
					std::vector<int> vec3;
				...
				else if (vec.size() == 8 && vec[0] == "location")
				{
					...
					loc.url = vec[1];
					loc.root = vec[2];
					std::vector<std::string> indexvec = split(vec[3], ',');
					loc.index = indexvec;
					std::vector<std::string> methodsvec = split(vec[4], ',');
					...
					std::vector<std::string> cgivec = split(vec[6], ',');
					try{
					...
					std::vector<std::string> uploadvec = split(vec[7], ',');
					try{
				...
				else if (vec.size() == 3 && vec[0] == "location")
				{
					...
					loc.url = vec[1];
					try{
						std::vector<std::string> redirvec = split(vec[2], ',');
